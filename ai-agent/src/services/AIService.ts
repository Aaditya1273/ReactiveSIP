...
Respond in JSON format:
{
  ...
  async generateOptimizationSuggestions(portfolio: any, plans: any[]): Promise<string> {
    try {
      const prompt = `Analyze this DeFi portfolio and provide optimization suggestions:
...
Be specific and practical.`;
      ...
      const response = await this.openai.chat.completions.create({
        model: this.model,
        messages: [
          { role: 'system', content: 'You are a DeFi portfolio optimization expert.' },
  ...
  async generateResponse(message: string, userAddress: string): Promise<string> {
    try {
      const systemPrompt = `You are a helpful AI assistant for ReactiveSIP, a DeFi platform for automated Systematic Investment Plans (SIP) on Reactive Network.
...
Always encourage smart investing and risk management.`;
      ...
      return response.choices[0].message.content || 'I apologize, but I couldn\'t process that. Could you rephrase?';
    } catch (error: any) {
  ...
  async generateRiskAssessment(portfolio: any): Promise<string> {
    try {
      const prompt = `Assess the risk level of this DeFi portfolio:
...
Be concise and actionable.`;
      ...
      const response = await this.openai.chat.completions.create({
        model: this.model,
        messages: [
          { role: 'system', content: 'You are a DeFi risk assessment expert.' },
          { role: 'user', content: prompt }
        ],
        temperature: 0.5,
        max_tokens: 300
      ...
      return response.choices[0].message.content || 'Unable to assess risk at this time.';
    } catch (error: any) {
